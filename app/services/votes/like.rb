module Votes
  class Like < Votes::Base
    ...
    def call
      validate!
      update_item(item) do
        item.disliked_user_ids.include?(user.id) ? item.user.increment!(:rating, 2) : item.user.increment!(:rating)
        item.liked_user_ids << user.id
    ...
    def validate!
      ...
      item_valid?(item)
      item_already_liked?(item, user.id)
    end
  end
end
